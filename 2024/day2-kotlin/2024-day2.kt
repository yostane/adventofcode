...
fun isReportSafeWithDampener(reportLevels: List<Int>): Boolean {
    val dampenedPossibilities = (0..<(reportLevels.size)).map {
        reportLevels.filterIndexed { index, item -> index != it}
    }
    return dampenedPossibilities.any { isReportSafe(it)  }
}
